...
subtitle: "FOURCAST legacy data publication: Dolichopodidae"
...
```{r}
input_data1 <- read_excel(here::here("datasets", "dolichopodidae", "data", "raw", "Dolichopodidae_Total_JV_TC_diB.xlsx"))

```

```{r}
input_data2 <- read_excel(here::here("datasets", "dolichopodidae", "data", "raw", "EB_Site_Table.xlsx"))
...
```{r}
event %<>% mutate(dwc_datasetName = "FSFN: Forest soil classification - Dolichopodidae, Flanders, Belgium") # e.g. "Checklist of non-native freshwater fishes in Flanders, Belgium"
```
...
```{r}
write_csv(event, here::here("datasets", "dolichopodidae", "data", "processed", "event.csv"), na = "")
```
...
occurrence %<>% mutate(occurrenceID = row_number()) %<>%
                mutate(dwc_occurrenceID = str_c("SOIL:DOLICHOPODIDAE:", occurrenceID))
...
```{r}
occurrence %<>% mutate(dwc_recordedBy = 'Domir De Bakker| Konjev Desender| Patrick Grootaert')
```
...
```{r}
occurrence %<>% mutate(dwc_identifiedBy = 'Domir De Bakker| Konjev Desender| Patrick Grootaert')
```
### CollectionCode
...
```{r}
write_csv(occurrence, here::here("datasets", "dolichopodidae", "data", "processed", "occurrence.csv"), na = "")
```
