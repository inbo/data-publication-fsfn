...
subtitle: "FOURCAST legacy data publication: Chilopoda"
...
```{r}
input_data1 <- read_excel(here::here("datasets", "chilopoda", "data", "raw", "Chilopoda_Total_JV_TC_diB.xlsx"))

```

```{r}
input_data2 <- read_excel(here::here("datasets", "chilopoda", "data", "raw", "EB_Site_Table.xlsx"))
...
```{r}
event %<>% mutate(dwc_datasetName = "FSFN: Forest soil classification - Chilopoda, Flanders, Belgium") # e.g. "Checklist of non-native freshwater fishes in Flanders, Belgium"
```
...
```{r}
write_csv(event, here::here("datasets", "chilopoda", "data", "processed", "event.csv"), na = "")
```
...
occurrence %<>% mutate(occurrenceID = row_number()) %<>%
                mutate(dwc_occurrenceID = str_c("SOIL:CHILOPODA:", occurrenceID))
...
```{r}
occurrence %<>% mutate(dwc_recordedBy = 'Leen Van den Haute')
```
...
```{r}
occurrence %<>% mutate(dwc_identifiedBy = 'Leen Van den Haute')
```
### CollectionCode
...
```{r}
write_csv(occurrence, here::here("datasets", "chilopoda", "data", "processed", "occurrence.csv"), na = "")
```
