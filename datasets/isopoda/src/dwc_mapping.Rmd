...
subtitle: "FOURCAST legacy data publication: Isopoda"
...
```{r}
input_data1 <- read_excel(here::here("datasets", "isopoda", "data", "raw", "Isopoda_Total_JV_TC_diB.xlsx"))

```

```{r}
input_data2 <- read_excel(here::here("datasets", "isopoda", "data", "raw", "EB_Site_Table.xlsx"))
...
```{r}
event %<>% mutate(dwc_datasetName = "FSFN: Forest soil classification - Isopoda, Flanders, Belgium") # e.g. "Checklist of non-native freshwater fishes in Flanders, Belgium"
```
...
```{r}
write_csv(event, here::here("datasets", "isopoda", "data", "processed", "event.csv"), na = "")
```
...
occurrence %<>% mutate(occurrenceID = row_number()) %<>%
                mutate(dwc_occurrenceID = str_c("SOIL:ISOPODA:", occurrenceID))
...
```{r}
occurrence %<>% mutate(dwc_recordedBy = 'Nathalie Devaere')
```
...
```{r}
occurrence %<>% mutate(dwc_identifiedBy = 'Nathalie Devaere')
```
### CollectionCode
...
```{r}
write_csv(occurrence, here::here("datasets", "isopoda", "data", "processed", "occurrence.csv"), na = "")
```
