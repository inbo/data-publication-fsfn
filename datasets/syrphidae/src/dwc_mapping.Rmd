...
subtitle: "FOURCAST legacy data publication: Syrphidae"
...
```{r}
input_data1 <- read_excel(here::here("datasets", "syrphidae", "data", "raw", "Syrphidae_Total_TC_DiB.xlsx"))

```

```{r}
input_data2 <- read_excel(here::here("datasets", "syrphidae", "data", "raw", "EB_Site_Table.xlsx"))
...
```{r}
event %<>% mutate(dwc_datasetName = "FSFN: Forest soil classification - Syrphidae, Flanders, Belgium") # e.g. "Checklist of non-native freshwater fishes in Flanders, Belgium"
```
...
```{r}
event %<>% mutate(dwc_samplingProtocol = 'pitfall trap') 
...
```{r}
event %<>% mutate(dwc_samplingEffort = 'pitfall trap')
...
```{r}
event %<>% mutate(dwc_verbatimSRS = "Belgiun Datum 1972")
```
...
```{r}
write_csv(event, here::here("datasets", "syrphidae", "data", "processed", "event.csv"), na = "")
```
...
occurrence %<>% mutate(occurrenceID = row_number()) %<>%
                mutate(dwc_occurrenceID = str_c("SOIL:SYRPHIDAE:", occurrenceID))
...
```{r}
occurrence %<>% mutate(dwc_recordedBy = 'Luc De Bruyn | Sofie Thys | Luc Verlinden')
```
...
```{r}
occurrence %<>% mutate(dwc_identifiedBy = 'Luc De Bruyn | Sofie Thys | Luc Verlinden')
```
### CollectionCode
...
```{r}
occurrence %<>% mutate(
  dwc_scientificName = case_when(
    scientificName == "Pipiza luteilabris" ~ "Pipiza luteitarsis",
    TRUE ~ scientificName
  )
)
```
...
```{r}
write_csv(occurrence, here::here("datasets", "syrphidae", "data", "processed", "occurrence.csv"), na = "")
```
