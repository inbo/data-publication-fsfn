...
subtitle: "FOURCAST legacy data publication: Lonchopteridae"
...
```{r}
input_data1 <- read_excel(here::here("datasets", "lonchopteridae", "data", "raw", "lonchopteridae_Total_TC_DiB.xlsx"))

```

```{r}
input_data2 <- read_excel(here::here("datasets", "lonchopteridae", "data", "raw", "EB_Site_Table.xlsx"))
...
```{r}
event %<>% mutate(dwc_datasetName = "FSFN: Forest soil classification - Lonchopteridae, Flanders, Belgium") # e.g. "Checklist of non-native freshwater fishes in Flanders, Belgium"
```
...
```{r}
write_csv(event, here::here("datasets", "lonchopteridae", "data", "processed", "event.csv"), na = "")
```
...
occurrence %<>% mutate(occurrenceID = row_number()) %<>%
                mutate(dwc_occurrenceID = str_c("SOIL:LONCHOPTERIDAE:", occurrenceID))
...
```{r}
write_csv(occurrence, here::here("datasets", "lonchopteridae", "data", "processed", "occurrence.csv"), na = "")
```
