...
subtitle: "FOURCAST legacy data publication: Vegetation"
...
```{r}
input_data1 <- read_excel(here::here("datasets", "vegetation", "data", "raw", "Vegetation_TC_DiB.xlsx"))

```

```{r}
input_data2 <- read_excel(here::here("datasets", "vegetation", "data", "raw", "EB_Site_Table.xlsx"))
...
```{r}
event %<>% mutate(dwc_datasetName = "FSFN: Forest soil classification - Vegetation, Flanders, Belgium") # e.g. "Checklist of non-native freshwater fishes in Flanders, Belgium"
```
...
```{r}
event %<>% mutate(dwc_samplingProtocol = 'observation') 
...
```{r}
event %<>% mutate(dwc_samplingEffort = 'area search')
...
```{r}
write_csv(event, here::here("datasets", "vegetation", "data", "processed", "event.csv"), na = "")
```
...
occurrence %<>% mutate(occurrenceID = row_number()) %<>%
                mutate(dwc_occurrenceID = str_c("SOIL:VEGETATION:", occurrenceID))
...
```{r}
occurrence %<>% mutate(dwc_recordedBy = 'Domir De Bakker| Konjev Desender | Frederik Hendrickx')
```
...
```{r}
occurrence %<>% mutate(dwc_identifiedBy = 'Domir De Bakker| Konjev Desender | Frederik Hendrickx')
```
...
```{r}
write_csv(occurrence, here::here("datasets", "vegetation", "data", "processed", "occurrence.csv"), na = "")
```
