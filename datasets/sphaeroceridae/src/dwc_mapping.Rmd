...
subtitle: "FOURCAST legacy data publication: Sphaeroceridae"
...
```{r}
input_data1 <- read_excel(here::here("datasets", "sphaeroceridae", "data", "raw", "sphaeroceridae_Total_TC_diB.xlsx"))

```

```{r}
input_data2 <- read_excel(here::here("datasets", "sphaeroceridae", "data", "raw", "EB_Site_Table.xlsx"))
...
```{r}
event %<>% mutate(dwc_datasetName = "FSFN: Forest soil classification - Sphaeroceridae, Flanders, Belgium") # e.g. "Checklist of non-native freshwater fishes in Flanders, Belgium"
```
...
```{r}
write_csv(event, here::here("datasets", "sphaeroceridae", "data", "processed", "event.csv"), na = "")
```
...
occurrence %<>% mutate(occurrenceID = row_number()) %<>%
                mutate(dwc_occurrenceID = str_c("SOIL:SPHAEROCERIDAE:", occurrenceID))
...
```{r}
occurrence %<>% mutate(dwc_recordedBy = 'Luc De Bruyn | Sofie Thys | Bart Engelen')
```
...
```{r}
occurrence %<>% mutate(dwc_identifiedBy = 'Luc De Bruyn | Sofie Thys | Bart Engelen')
```
...
```{r}
write_csv(occurrence, here::here("datasets", "sphaeroceridae", "data", "processed", "occurrence.csv"), na = "")
```
