...
subtitle: "FOURCAST legacy data publication: Collembola"
...
```{r}
event %<>% mutate(dwc_datasetID = "https://doi.org/10.15468/tkj5tj")
```
### type
...
```{r}
#occurrence %<>% mutate(datasetID = "https://doi.org/10.15468/tkj5tj") 
```
...
```{r}
event %<>% mutate(dwc_datasetName = "FSFN: Forest soil classification - Collembola, Flanders, Belgium") # e.g. "Checklist of non-native freshwater fishes in Flanders, Belgium"
```
...
```{r}
event %<>% mutate(dwc_locality = `Observation Site`) 
```
...
```{r}
event %<>% mutate(dwc_verbatimcoordinatesystem = "Belgian Lambert 72") # 
```

### verbatimSRS

```{r}
event %<>% mutate(dwc_verbatimSRS = "Belgium Date 1972")
```
...
```{r}
write_csv(event, here::here("datasets", "Collembola", "data", "processed", "event.csv"), na = "")
```
...
occurrence %<>% mutate(occurrenceID = row_number()) %<>%
                mutate(dwc_occurrenceID = str_c("SOIL:COLLEMBOLA:OCC:", occurrenceID))
...
mutate(dwc_eventID = str_c("SOIL:COLLEMBOLA:EV", dwc_eventDate, Site ))
...
```
```{r}
occurrence %<>% filter(dwc_scientificName != 'Som')
```
