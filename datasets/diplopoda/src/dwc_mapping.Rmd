...
subtitle: "FOURCAST legacy data publication: Diplopoda"
...
```{r}
input_data1 <- read_excel(here::here("datasets", "diplopoda", "data", "raw", "Diplopoda_Total_JV_TC_diB.xlsx"))

```

```{r}
input_data2 <- read_excel(here::here("datasets", "diplopoda", "data", "raw", "EB_Site_Table.xlsx"))
...
```{r}
event %<>% mutate(dwc_datasetName = "FSFN: Forest soil classification - Diplopoda, Flanders, Belgium") # e.g. "Checklist of non-native freshwater fishes in Flanders, Belgium"
```
...
```{r}
write_csv(event, here::here("datasets", "diplopoda", "data", "processed", "event.csv"), na = "")
```
...
occurrence %<>% mutate(occurrenceID = row_number()) %<>%
                mutate(dwc_occurrenceID = str_c("SOIL:DIPLOPODA:", occurrenceID))
...
```{r}
write_csv(occurrence, here::here("datasets", "diplopoda", "data", "processed", "occurrence.csv"), na = "")
```
