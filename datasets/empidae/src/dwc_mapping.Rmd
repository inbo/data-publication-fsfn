...
subtitle: "FOURCAST legacy data publication: empidae"
...
```{r}
input_data1 <- read_excel(here::here("datasets", "empidae", "data", "raw", "Empidae_Total_TC_Dib.xlsx"))

```

```{r}
input_data2 <- read_excel(here::here("datasets", "empidae", "data", "raw", "EB_Site_Table.xlsx"))
...
```{r}
event %<>% mutate(dwc_datasetName = "FSFN: Forest soil classification - Empidae, Flanders, Belgium") # e.g. "Checklist of non-native freshwater fishes in Flanders, Belgium"
```
...
```{r}
write_csv(event, here::here("datasets", "empidae", "data", "processed", "event.csv"), na = "")
```
...
occurrence %<>% mutate(occurrenceID = row_number()) %<>%
                mutate(dwc_occurrenceID = str_c("SOIL:EMPIDAE:", occurrenceID))
...
```{r}
occurrence %<>% mutate(dwc_recordedBy = 'Luc De Bruyn | Sofie Thys | Mihaly Foldvari | Patrick Grootaert')
```
...
```{r}
occurrence %<>% mutate(dwc_identifiedBy = 'Luc De Bruyn | Sofie Thys| Mihaly Foldvari | Patrick Grootaert')
```
### CollectionCode
...
```{r}
write_csv(occurrence, here::here("datasets", "empidae", "data", "processed", "occurrence.csv"), na = "")
```
